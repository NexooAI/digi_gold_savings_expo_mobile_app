...
import BrandBConfig from './Config';

export default function BrandBScreen() {
  ...
  const { socket, handleCancel } = usePaymentSocket({
    onPaymentSuccess: (data) => {
      console.log('BrandB Payment Success:', data);
      ...
      router.replace({
        ...
        params: {
          ...
          brandName: BrandBConfig.brandName
        },
      });
    },
    onPaymentFailure: (data) => {
      console.log('BrandB Payment Failure:', data);
    ...
    onPaymentError: (error) => {
      console.error('BrandB Payment Error:', error);
  ...
  const handlePayNow = async () => {
    try {
      ...
      // Call BrandB's create-payment API
      const response = await fetch(BrandBConfig.apiEndpoint, {
        method: 'POST',
        headers: BrandBConfig.headers,
        body: JSON.stringify({
          amount: 1500, // Replace with actual amount
          currency: 'INR',
          customerId: 'user456', // Replace with actual customer ID
          description: 'BrandB Payment'
        })
      });
      ...
      const fullPaymentUrl = `${BrandBConfig.basePaymentUrl}/payment/${txnId}`;
      setPaymentUrl(fullPaymentUrl);
      setShowWebView(true);

    } catch (error) {
      console.error('Error creating BrandB payment:', error);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{BrandBConfig.brandName} Payment</Text>
