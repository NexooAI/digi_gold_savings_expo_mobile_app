...
import BrandCConfig from './Config';

export default function BrandCScreen() {
  ...
  const { socket, handleCancel } = usePaymentSocket({
    onPaymentSuccess: (data) => {
      console.log('BrandC Payment Success:', data);
      ...
      router.replace({
        ...
        params: {
          ...
          brandName: BrandCConfig.brandName
        },
      });
    },
    onPaymentFailure: (data) => {
      console.log('BrandC Payment Failure:', data);
    ...
    onPaymentError: (error) => {
      console.error('BrandC Payment Error:', error);
  ...
  const handlePayNow = async () => {
    try {
      ...
      // Call BrandC's create-payment API
      const response = await fetch(BrandCConfig.apiEndpoint, {
        method: 'POST',
        headers: BrandCConfig.headers,
        body: JSON.stringify({
          amount: 2000, // Replace with actual amount
          currency: 'INR',
          customerId: 'user789', // Replace with actual customer ID
          description: 'BrandC Payment'
        })
      });
      ...
      const fullPaymentUrl = `${BrandCConfig.basePaymentUrl}/payment/${txnId}`;
      setPaymentUrl(fullPaymentUrl);
      setShowWebView(true);

    } catch (error) {
      console.error('Error creating BrandC payment:', error);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{BrandCConfig.brandName} Payment</Text>
