...
import DCJewellersConfig from './Config';

export default function DCJewellersScreen() {
  ...
  const { socket, handleCancel } = usePaymentSocket({
    onPaymentSuccess: (data) => {
      console.log('DCJewellers Payment Success:', data);
      ...
      router.replace({
        ...
        params: {
          ...
          brandName: DCJewellersConfig.brandName
        },
      });
    },
    onPaymentFailure: (data) => {
      console.log('DCJewellers Payment Failure:', data);
    ...
    onPaymentError: (error) => {
      console.error('DCJewellers Payment Error:', error);
  ...
  const handlePayNow = async () => {
    try {
      ...
      // Call DCJewellers's create-payment API
      const response = await fetch(DCJewellersConfig.apiEndpoint, {
        method: 'POST',
        headers: DCJewellersConfig.headers,
        body: JSON.stringify({
          amount: 1500, // Replace with actual amount
          currency: 'INR',
          customerId: 'user456', // Replace with actual customer ID
          description: 'DCJewellers Payment'
        })
      });
      ...
      const fullPaymentUrl = `${DCJewellersConfig.basePaymentUrl}/payment/${txnId}`;
      setPaymentUrl(fullPaymentUrl);
      setShowWebView(true);

    } catch (error) {
      console.error('Error creating DCJewellers payment:', error);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{DCJewellersConfig.brandName} Payment</Text>
