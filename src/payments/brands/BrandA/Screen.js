...
import BrandAConfig from './Config';

export default function BrandAScreen() {
  ...
  const { socket, handleCancel } = usePaymentSocket({
    onPaymentSuccess: (data) => {
      console.log('BrandA Payment Success:', data);
      ...
      router.replace({
        ...
        params: {
          ...
          brandName: BrandAConfig.brandName
        },
      });
    },
    onPaymentFailure: (data) => {
      console.log('BrandA Payment Failure:', data);
    ...
    onPaymentError: (error) => {
      console.error('BrandA Payment Error:', error);
  ...
  const handlePayNow = async () => {
    try {
      ...
      // Call BrandA's create-payment API
      const response = await fetch(BrandAConfig.apiEndpoint, {
        method: 'POST',
        headers: BrandAConfig.headers,
        body: JSON.stringify({
          amount: 1000, // Replace with actual amount
          currency: 'INR',
          customerId: 'user123', // Replace with actual customer ID
          description: 'BrandA Payment'
        })
      });
      ...
      const fullPaymentUrl = `${BrandAConfig.basePaymentUrl}/payment/${txnId}`;
      setPaymentUrl(fullPaymentUrl);
      setShowWebView(true);

    } catch (error) {
      console.error('Error creating BrandA payment:', error);
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.title}>{BrandAConfig.brandName} Payment</Text>
