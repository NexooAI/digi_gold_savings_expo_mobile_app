...
export default function PaymentProcessScreen() {
  ...
  const finalUserDetails = useMemo<UserDetails>(() => {
    if (Object.keys(parsedUserDetails).length > 0) {
      ...
      return {
        ...
        retryData: userDetails.isRetryAttempt
          ? userDetails.retryData
          : undefined,
      };
    }
  ...
  const handlePaymentSuccess = useCallback(async (data: PaymentStatusUpdate) => {
    try {
      setIsLoading(true);
      navigation.setOptions({
        tabBarStyle: { display: "flex" },
      ...
      router.push({
        ...
        params: {
          amount: data?.paymentResponse?.amount || paymentDetails.amount,
  ...
  const handlePaymentFailure = useCallback(async (data: PaymentStatusUpdate) => {
    try {
      setIsLoading(true);
      navigation.setOptions({
        tabBarStyle: { display: "flex" },
      ...
      router.push({
        ...
        params: {
          ...
          dateTime: currentDate,
          status: "Failed",
          errorMessage: data?.paymentResponse?.payment_gateway_response?.resp_message || "Payment failed"
        },
      });
    } catch (error) {
  ...
  }, [router, navigation, paymentDetails]);
  ...
  const handlePaymentError = useCallback((error: any) => {
    ...
    const errorMessage = error?.error || error?.message || "An error occurred while processing the transaction.";
    const errorTimestamp = error?.timestamp || new Date().toISOString();
    const errorCode = error?.error?.includes("HTTP Error:") ? error.error.split("HTTP Error:")[1].trim() : "Unknown";

    router.push({
      pathname: "/(tabs)/home/payment-failure",
      params: {
        amount: error?.paymentResponse?.amount || paymentDetails.amount,
    ...
    const loadSessionData = async () => {
      try {
        ...
        if (storedSessionData) {
          ...
          if (
            isRetryFromParams ||
            storedSessionData.userDetails?.isRetryAttempt
          ) {
            setIsRetry(true);
          }
        }
  ...
  const handlePayPress = useCallback(async () => {
    try {
      ...
      const payload: PaymentInitPayload = {
        userId:
          finalUserDetails.userId || finalUserDetails.data?.data?.userId || "",
        amount: amount,
        investmentId:
          finalUserDetails.investmentId ||
          finalUserDetails.data?.data?.id ||
          "",
        schemeId:
          finalUserDetails.schemeId ||
          finalUserDetails.data?.data?.schemeId ||
          "",
        userEmail: finalUserDetails.email || finalUserDetails.userEmail || "",
        userMobile:
          finalUserDetails.mobile || finalUserDetails.userMobile || "",
        userName: finalUserDetails.name || finalUserDetails.accountname || "",
        chitId:
          finalUserDetails.chitId || finalUserDetails.data?.data?.chitId || 1,
      };
    ...
    try {
      ...
      if (paymentRetryData) {
        setPaymentDetails((prev: PaymentState) => ({
          ...
          schemeName:
            paymentRetryData.displayData.schemeName || "Gold Savings Scheme",
          installmentNumber:
            Number(paymentRetryData.displayData.monthsPaid) + 1 || 1,
            ...
            <View style={styles.schemeInfoCard}>
              <View style={styles.schemeHeader}>
                ...
                <Text style={styles.schemeName}>
                  {paymentDetails.schemeName} test
                </Text>
              </View>
              <View style={styles.schemeDetails}>
                <View style={styles.schemeDetailItem}>
                  ...
                  <Text style={styles.schemeDetailValue}>
                    {paymentDetails.installmentNumber} of{" "}
